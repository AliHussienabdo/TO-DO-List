...
function populateProjectsFromLocalStorage(){
    ...
    let TheProjects = JSON.parse(localStorage.getItem('projects'));

    let allTheProjects = [];
    TheProjects.forEach(project => {
        let list = new List(project.name);
        ...
        project.tasksList.forEach(tsk => {
            let TASK = new task(tsk.title, tsk.dueDate, tsk.priority, tsk.completed);
            list.addTask(TASK);
            if(list.name == 'Home'){
